...
weights_name = f'{datetime.date.today().isoformat()}_resnet_oan_lr_lr{start_lr}_step{num_steps}_wce_weight{wce_weight}'
path_to_save = f'/home/maantonov_1/VKR/weights/resnet_oan/main_copypaste_3_people/{datetime.date.today().isoformat()}/wce_weight{wce_weight}/'
...
transform = nn.Sequential(
    kornia.augmentation.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.2, hue = 0.01, p=0.3),
...
cp = CopyPasteAugmentation(path, max_objects = 3, random_state = 42, feather_amount = 31, bb_pad = 0.0)
...
criterion = MSE_WCE_IOU(w = wce_weight)
...
def train_one_epoch(epoch_num, train_data_loader):
    ...
    for iter_num, data in enumerate(train_data_loader):
        ...
        loss_mse,  loss_ce, loss_iou = criterion(pred, mask)
...
def get_metric_one_epoch(epoch_num, valid_data_loader, best_val):
    ...
    for iter_num, data in enumerate(valid_data_loader):
        ...
        with torch.no_grad():
            ...
            loss_mse,  loss_ce, loss_iou = criterion(pred1, mask)
