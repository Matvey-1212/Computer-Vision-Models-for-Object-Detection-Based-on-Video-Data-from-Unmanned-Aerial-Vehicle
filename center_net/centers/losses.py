...
def _reg_loss(regr, gt_regr, mask):
    num = torch.clip(mask.float().sum(), 0.00001)
    #print(gt_regr.size())
...
def centerloss(prediction, mask, regr,weight=0.4, size_average=True):
    ...
    regr_loss = (torch.abs(pred_regr - regr).sum(1) * mask).sum(1).sum(1) / torch.clip(mask.sum(1).sum(1),0.00001)
...
def centerloss2(prediction, mask, weight=0.4, size_average=True):
    # Binary mask loss
    pred_mask = torch.sigmoid(prediction[:, 0])
    mask_loss = neg_loss(pred_mask, mask)
...
# predicted = np.array([[1, 2], [3, 4], [6, 7]])
# true = np.array([[1, 10], [3.1, 4], [8, 9]])

# precision, recall, f1 = calculate_accuracy_metrics(predicted, true, threshold=3)
