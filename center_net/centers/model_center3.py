...
class double_conv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_ch, (in_ch + out_ch)//2, 3, padding=1),
            nn.BatchNorm2d((in_ch + out_ch)//2),
            nn.ReLU(inplace=True),
            nn.Conv2d((in_ch + out_ch)//2, out_ch, 3, padding=1),
...
class centernet(nn.Module):
    def __init__(self, n_classes=1, model_name="resnet18"):
        ...
        self.up1 = double_conv(num_ch, 256)
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes, block, layers, inputs = 3):
        ...
        self.up1 = double_conv(fpn_sizes[2], 256)
        self.outc = nn.Conv2d(256, n_classes, 1)
