...
weights_name = f'{datetime.date.today().isoformat()}_{model_name}_main_lr_lr{start_lr}_step{num_steps}_wce_weight{wce_weight}'
path_to_save = f'/home/maantonov_1/VKR/weights/{model_name}/main/{datetime.date.today().isoformat()}/wce_weight{wce_weight}/'
...
model = MobileNetV3Seg(2, backbone=model_name, pretrained_base=False, aux=False, attention = True)
...
def get_metric_one_epoch(epoch_num, valid_data_loader, best_val):
    ...
    for iter_num, data in enumerate(valid_data_loader):
        ...
        pred = torch.argmax(pred1, dim=1)[0].cpu().long()
        mask = mask.cpu()
