...
from retinanet_new.utils import BasicBlock, Bottleneck, BBoxTransform, ClipBoxes
from retinanet_new.anchors import Anchors
...
class PyramidFeatures(nn.Module):
    def __init__(self, C3_size, C4_size, C5_size, feature_size=256):
        ...
        self.P2_1 = nn.Conv2d(C3_size // 2, feature_size, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, inputs):
        ...
        # P4_x = torch.cat((P4_x, P5_upsampled_x), dim=1)
        ...
        P2_x = self.P2_1(C2) #128
        P2_x = P2_x + P3_upsampled_x #128
        # P2_x = torch.cat((P2_x, P3_upsampled_x), dim=1)
        P2_x = self.P2_2(P2_x) #128
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes, block, layers, inputs = 3, oan_gamma = 2, oan_alpha = 0.25):
        ...
        self.focalLoss = losses.FocalLoss(alpha = oan_alpha, gamma = oan_gamma)
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            return class_loss, reg_loss, oan_loss, x_oan
        else:
...
def resnet50(num_classes, pretrained=False, inputs = 3,  oan_gamma = 2, oan_alpha = 0.25, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(num_classes, Bottleneck, [3, 4, 6, 3], inputs = inputs,  oan_gamma = 2, oan_alpha = 0.25, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50'], model_dir='.'), strict=False)
    return model


def resnet101(num_classes, pretrained=False, oan_gamma = 2, oan_alpha = 0.25, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(num_classes, Bottleneck, [3, 4, 23, 3], oan_gamma = 2, oan_alpha = 0.25, **kwargs)
    if pretrained:
