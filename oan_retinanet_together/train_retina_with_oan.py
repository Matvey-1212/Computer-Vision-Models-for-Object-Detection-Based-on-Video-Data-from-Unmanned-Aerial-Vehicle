...
# wandb.init(project="VKR", entity="matvey_antonov", name = "retina_with_oan")
...
optimizer = torch.optim.SGD(retinanet.parameters(), lr = 0.0003)
...
def train_one_epoch(epoch_num, train_data_loader):
    ...
    for iter_num, data in enumerate(train_data_loader):
        ...
        classification_loss, regression_loss, map_loss = retinanet([img, annot])
        ...
        loss = classification_loss + regression_loss + map_loss
        ...
        print(
            'Epoch: {} | Iteration: {} | Classification loss: {:1.5f} | Regression loss: {:1.5f} | Map loss: {:1.5f} | Running loss: {:1.5f}'.format(
                epoch_num, iter_num, float(classification_loss), float(regression_loss), float(map_loss), np.mean(epoch_loss)), flush=True)
...
def valid_one_epoch(epoch_num, valid_data_loader):
    ...
    torch.save(retinanet, f"oan_only_{np.mean(epoch_loss)}.pt")
    return np.mean(epoch_loss)
