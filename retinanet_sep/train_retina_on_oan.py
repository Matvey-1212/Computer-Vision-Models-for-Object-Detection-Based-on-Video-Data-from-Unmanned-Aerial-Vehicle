...
wandb.init(project="VKR", entity="matvey_antonov", name = "retina_on_oan")
...
oan = OAN.OAN();
retinanet = model.resnet50(num_classes = 1, pretrained = False, inputs = 4)
...
upsample = torch.nn.Upsample(scale_factor=64) 
...
def train_one_epoch(epoch_num, train_data_loader):
    ...
    for iter_num, data in enumerate(train_data_loader):
        ...
        with torch.no_grad():
            ...
            x = upsample(x)
            x = torch.cat([img, x], dim=1).detach()
        
        
        classification_loss, regression_loss = retinanet([x, annot])
