...
#         if rotate[i] == 90:
#             transformed = transform_90(image=np.zeros_like(image),bboxes=bb_t, class_labels=[bbox[4] for bbox in bboxes])
#             transformed_bboxes = transformed['bboxes']
            
#         elif rotate[i] == -90:
#             transformed = transform_270(image=np.zeros_like(image),bboxes=bb_t, class_labels=[bbox[4] for bbox in bboxes])
#             transformed_bboxes = transformed['bboxes']
            
#         bb[i, :shapes[i][0]] = bb_t
...
@pipeline_def( device_id=0)
def get_dali_pipeline_no_resize(images_dir, annotations_file, resize_dims=(1024, 1024), seed = 42):
    inputs, bboxes, labels, img_id = fn.readers.coco(
        file_root=images_dir,
        annotations_file=annotations_file,
        ltrb=True,
