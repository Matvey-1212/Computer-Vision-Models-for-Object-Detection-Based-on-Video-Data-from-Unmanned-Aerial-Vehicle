...
from utils.Dali_resize import get_dali_pipeline, get_dali_pipeline_aug, flip_bboxes2, flip_bboxes, resize_bb, get_dali_pipeline_two_stages, get_dali_pipeline_two_stages_faster
...
dali_iterator_two_stages = DALIGenericIterator(
    pipelines=[get_dali_pipeline_two_stages_faster(images_dir = images_dir_val, annotations_file = annotations_file_val,resize_dims = resize_to, batch_size = 1, num_threads = num_workers)],
...
path_to_weights1 = '/home/maantonov_1/VKR/weights/faster/resize/main/2024-04-11/2024-04-11_faster_resize_h:1024_w:1024_small+main_lr:0.0003_step:10_n29_m:0.61_f:0.43_val:0.0293.pt'
weights1 = '/home/maantonov_1/VKR/weights/faster/from_slast/big_model.ckpt'

retinanet = torch.load(path_to_weights1, map_location=device)
weights = torch.load(weights1,map_location=torch.device('cpu'))
print(retinanet.load_state_dict(weights, strict=True))
retinanet.to(device)
retinanet.eval()
...
for iter_num, data in enumerate(dali_iterator_two_stages):
    ...
    with torch.no_grad():
        ...
        images_resized = data[0]['images_resized']/255
        annot = data[0]['bboxes']
        ...
        if len(scores) != 0:
            ...
            centers = np.zeros((scores.shape[0],2))
...
pd.DataFrame(active_list, columns = ['id','sccore','xmin','ymin','xmax','ymax','map','fscore']).to_csv('/home/maantonov_1/VKR/actual_scripts/retinanet_sep/AL/AL_on_test_two_stages_faster.csv')
