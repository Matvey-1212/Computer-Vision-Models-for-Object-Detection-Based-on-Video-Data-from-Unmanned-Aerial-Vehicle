...
weights_name = f'{datetime.date.today().isoformat()}_retinanet101_oan_new_fpn_ul_lr:{start_lr}_step:{num_steps}_gamma:{oan_gamma}_alpha:{oan_alpha}'
...
path_to_save = f'/home/maantonov_1/VKR/weights/retinanet_new_fpn/{datetime.date.today().isoformat()}/gamma{oan_gamma}_alpha{oan_alpha}/'
...
retinanet = model_oan_new_fpn_layer.resnet50(num_classes = 2, pretrained = False, inputs = 3, oan_gamma = oan_gamma, oan_alpha = oan_alpha)
# retinanet = model_oan_new_fpn_layer.resnet101(num_classes = 2, pretrained = False, inputs = 3, oan_gamma = oan_gamma, oan_alpha = oan_alpha)
