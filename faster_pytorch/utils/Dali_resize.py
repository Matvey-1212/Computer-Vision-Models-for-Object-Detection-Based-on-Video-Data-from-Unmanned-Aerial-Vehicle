...
def get_dali_pipeline(images_dir, annotations_file, resize_dims=(1024, 1024), seed = 42):
    ...
    res_images = fn.resize(images, resize_x=resize_dims[0], resize_y=resize_dims[1])
    ...
    images = fn.crop_mirror_normalize(images,)
                                    #   mean=mean,
                                    #   std=std, device='gpu')
    res_images = fn.crop_mirror_normalize(res_images,)
                                    #   mean=mean,
                                    #   std=std, device='gpu')
    
    return images,res_images,  bboxes, bbox_shapes, img_id, original_sizes
...
#     bbox = resize_bb(bbox, original_sizes, bb_pad = 0.1, new_shape = (1024, 1024))
